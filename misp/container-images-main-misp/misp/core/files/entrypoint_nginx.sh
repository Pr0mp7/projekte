...
init_misp_data_files(){
    ...
    # List of files to check and their default versions
    declare -A files=(
        ["bootstrap.php"]="bootstrap.default.php"
        ["database.php"]="database.default.php"
        ["core.php"]="core.default.php"
    ...
    for file in "${!files[@]}"; do
        if [ -f "$MISP_APP_CONFIG_PATH/$file" ]; then
            echo "File $MISP_APP_CONFIG_PATH/$file already exists."
        else
            echo "File $MISP_APP_CONFIG_PATH/$file does not exist. Copying default file..."
            if dd if="$MISP_APP_CONFIG_PATH.dist/${files[$file]}" of="$MISP_APP_CONFIG_PATH/$file"; then
                echo "File $file was successfully copied."
            else
    ...
    # workarounds for https://forums.docker.com/t/sed-couldnt-open-temporary-file-xyz-permission-denied-when-using-virtiofs/125473
...
init_nginx() {
    ...
    : ${NGINX_LIVE:=false}
    : ${NGINX_RELOAD:=false}
    flip_nginx $NGINX_LIVE $NGINX_RELOAD
