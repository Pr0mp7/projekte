...
[ -z "$BACKUP_COUNT" ] && BACKUP_COUNT=90
...
* * * * * /var/www/MISP/read-misp-logs.sh $MYSQL_USER $MYSQL_HOST $MYSQL_DATABASE >> /var/www/MISP/app/tmp/logs/misp.log 2>&1
*/5 * * * * /misp-backup.sh backup $MYSQL_HOST $MYSQL_DATABASE $BACKUP_COUNT >> /var/www/MISP/app/tmp/logs/misp.backup.log 2>&1
