...
[ -z "$ENABLE_CSP_ENFORCE" ] && ENABLE_CSP_ENFORCE="true"
[ -z "$DO_NOT_LOG_AUTH_KEYS" ] && DO_NOT_LOG_AUTH_KEYS="true"
[ -z "$AUTH_AMOUNT" ] && AUTH_AMOUNT=5
[ -z "$AUTH_EXPIRE" ] && AUTH_EXPIRE=300
[ -z "$LOG_CLIENT_IP" ] && LOG_CLIENT_IP="true"
[ -z "$LOG_USER_IPS" ] && LOG_USER_IPS="true"
[ -z "$LOG_AUTH" ] && LOG_AUTH="true"
[ -z "$LOG_NEW_AUDIT" ] && LOG_NEW_AUDIT="true"
[ -z "$PASSWORD_LENGTH" ] && PASSWORD_LENGTH=3
[ -z "$PASSWORD_COMPLEXITY" ] && PASSWORD_COMPLEXITY='/.*/'
[ -z "$ENABLE_UPDATE" ] && ENABLE_UPDATE="true"
[ -z "$AUTH_ENFORCED" ] && AUTH_ENFORCED="true"
...
[ -z "$LDAP_ENABLED" ] && LDAP_ENABLED="false"
...
init_configuration(){
    ...
    /var/www/MISP/app/Console/cake Admin setSetting -q "Plugin.CustomAuth_custom_logout" "$CUSTOM_LOGOUT_URL"
    /var/www/MISP/app/Console/cake Admin setSetting -q "SecureAuth.amount" "$AUTH_AMOUNT"
    /var/www/MISP/app/Console/cake Admin setSetting -q "SecureAuth.expire" "$AUTH_EXPIRE"
    /var/www/MISP/app/Console/cake Admin setSetting -q "MISP.log_new_audit" "$LOG_NEW_AUDIT"
    /var/www/MISP/app/Console/cake Admin setSetting -q "MISP.log_auth" "$LOG_AUTH"
    /var/www/MISP/app/Console/cake Admin setSetting "Security.auth_enforced" "$AUTH_ENFORCED"
}
...
init_user() {
    ...
    if [ -n "$ADMIN_KEY" ]; then
        ...
        CHANGE_CMD=(/var/www/MISP/app/Console/cake User change_authkey -q 1 "${ADMIN_KEY}")
    elif [ -z "$ADMIN_KEY" ] && [ "$AUTOGEN_ADMIN_KEY" == "true" ]; then
        echo "... regenerating admin key (set \$ADMIN_KEY if you want it to change)"
        CHANGE_CMD=(/var/www/MISP/app/Console/cake User change_authkey -q 1)
    else
        echo "... admin user key auto generation disabled"
        
    fi

    if [[ -n "$SYNC_USER" && -n "$SYNC_USER_AUTHKEY" ]]; then
        echo "... add sync user '${SYNC_USER}'"
        echo "$SYNC_USER" | xargs -I {} app/Console/cake user create {} 5 1
        echo "... setting authkey for user '${SYNC_USER}'"
        echo "$SYNC_USER" | xargs -I {} sh -c 'app/Console/cake user change_authkey -q {} $SYNC_USER_AUTHKEY'
    else
        echo "No sync user added"
    fi
    ...
    if [ ! -z "$ADMIN_PASSWORD" ]; then
        ...
        /var/www/MISP/app/Console/cake Admin setSetting "Security.password_policy_length" "$PASSWORD_LENGTH"
        /var/www/MISP/app/Console/cake Admin setSetting -q "Security.password_policy_length" 1
        /var/www/MISP/app/Console/cake Admin setSetting -q "Security.password_policy_complexity" "${PASSWORD_COMPLEXITY}"
        /var/www/MISP/app/Console/cake User change_pw -q "${ADMIN_EMAIL}" "${ADMIN_PASSWORD}"
...
add_config_user(){
    /var/www/MISP/app/Console/cake user create -q "${CONFIG_USERNAME}" 1 1
    /var/www/MISP/app/Console/cake user change_authkey -q "${CONFIG_USERNAME}" "${CONFIG_USER_AUTHKEY}"
    /var/www/MISP/app/Console/cake user block admin
}

add_soar_users(){
    if [ -z "$SOAR_USERS" ]; then
        echo "SOAR_USERS ist nicht gesetzt oder leer."
        return
    fi
    if ! command -v jq >/dev/null 2>&1; then
        echo "jq wird benötigt, bitte installieren!"
        return
    fi

    echo "${SOAR_USERS}" | jq -c '.[]' | while read -r user; do
        email=$(echo "$user" | jq -r '.email')
        role_id=$(echo "$user" | jq -r '.role_id')
        org_id=$(echo "$user" | jq -r '.org_id')
        secret_key=$(echo "$user" | jq -r '.secret_key')

        app/Console/cake user create "$email" "$role_id" "$org_id"
        app/Console/cake user change_authkey -q "$email" "$(cat /vault/soar-users-secret/$secret_key)"
    done
}
...
apply_critical_fixes() {
    ...
    /var/www/MISP/app/Console/cake Admin setSetting "Security.do_not_log_authkeys" "$DO_NOT_LOG_AUTH_KEYS"
    /var/www/MISP/app/Console/cake Admin setSetting -q "Security.csp_enforce" $ENABLE_CSP_ENFORCE
    php /var/www/MISP/tests/modify_config.php modify "{
        \"Security\": {
...
apply_optional_fixes() {
    ...
    #/var/www/MISP/app/Console/cake Admin setSetting -q "MISP.org" "${ORGNAME}"

    /var/www/MISP/app/Console/cake Admin setSetting -q "MISP.log_client_ip" "$LOG_CLIENT_IP"
    /var/www/MISP/app/Console/cake Admin setSetting -q "MISP.log_user_ips" "$LOG_USER_IPS"
...
extra_objects() {
    [ -z "$EXTRA_OBJECTS_ENABLED" ] && EXTRA_OBJECTS_ENABLED="false"
    [ -z "$EXTRA_OBJECTS_MOUNTPATH" ] && EXTRA_OBJECTS_MOUNTPATH="/tmp/extra-objects"
    [ -z "$EXTRA_OBJECTS_OBJECTPATH" ] && EXTRA_OBJECTS_OBJECTPATH="/var/www/MISP/app/files/misp-objects/objects"
    [ -z "$EXTRA_OBJECTS_OBJECTSCHEMA" ] && EXTRA_OBJECTS_OBJECTSCHEMA="/tmp/objects-schema-override/schema_objects.json"
    MAX_WAIT=30
    WAIT_TIME=0

    while [ ! -d "$EXTRA_OBJECTS_MOUNTPATH" ]; do
      if [ "$WAIT_TIME" -ge "$MAX_WAIT" ]; then
        echo "Timeout: Quellverzeichnis $EXTRA_OBJECTS_MOUNTPATH existiert nicht. Beende Skript."
        exit 1
      fi
      echo "Quellverzeichnis $EXTRA_OBJECTS_MOUNTPATH existiert nicht. Warte 2 Sekunden..."
      sleep 2
      WAIT_TIME=$((WAIT_TIME+2))
    done
    for FILE in "$EXTRA_OBJECTS_MOUNTPATH"/*; do
    if [ -f "$FILE" ] || [ -L "$FILE" ]; then
      BASENAME=$(basename "$FILE")
      DEST_DIR="$EXTRA_OBJECTS_OBJECTPATH/$BASENAME"
      mkdir -p "$DEST_DIR"
      cp "$FILE" "$DEST_DIR/definition.json"
      echo "File $FILE copied to $DEST_DIR/definition.json"
    fi
    done
    cp /tmp/objects-schema-override/schema_objects.json /var/www/MISP/app/files/misp-objects/
}
...
if [ "$ENABLE_UPDATE" = "true" ]; then 
    ...
    echo "Update components disabled"
fi
...
if [ "$EXTRA_OBJECTS_ENABLED" = "true" ]; then
    echo "MISP | Add extra objects ..." && extra_objects
else
    echo "MISP | add extra objects disabled"
fi

echo "MISP | Mark instance live"
/var/www/MISP/app/Console/cake Admin live 1

echo "MISP | Restarting workers"
supervisorctl -u supervisor -p supervisor restart misp-workers:*

if [ -n "${CONFIG_USERNAME}" ]; then
    echo "MISP | Disabling initial admin user" && add_config_user
else
    echo "MISP | Initial admin user will not be disabled"
fi

if [ "$SOAR_USERS_ENABLED" = "true" ]; then
    echo "MISP | Adding SOAR users" && add_soar_users
else
    echo "MISP | SOAR users not enabled"
fi

echo "MISP is ready" >> /var/www/MISP/app/tmp/logs/misp.log

if [ "$MISP_ADVANCED_LOGGING" = "true" ]; then
  echo "Advanced logging is enabled."
  crontab /etc/cron.d/misp
else
  echo "Advanced logging is disabled."
fi

if [[ "$MISP_debug_logs" == "true" ]]; then
  [[ ! -f "/var/www/MISP/app/tmp/logs/debug.log" ]] && touch /var/www/MISP/app/tmp/logs/debug.log && echo -e "\nCreated debug log file!" || echo -e "\nDebug log file already exists."
  echo "\n=====> Streaming MISP Debug Log <====="
  tail -F /var/www/MISP/app/tmp/logs/debug.log &
fi

[[ ! -f "/var/www/MISP/app/tmp/logs/error.log" ]] && touch /var/www/MISP/app/tmp/logs/error.log && echo -e "\nCreated error log file!" || echo -e "\nError log file already exists."
[[ ! -f "/var/www/MISP/app/tmp/logs/misp.log" ]] && touch /var/www/MISP/app/tmp/logs/misp.log && echo -e "\nCreated misp log file!" || echo -e "\nMISP log file already exists."

echo "\n=====> Streaming MISP Logs <====="
tail -F /var/www/MISP/app/tmp/logs/error.log /var/www/MISP/app/tmp/logs/misp.log &
